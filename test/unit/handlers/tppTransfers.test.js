...
const Handler = require('../../../src/domain/tppTransfers')
...
describe('/tppTransfers', () => {
  ...
  beforeEach(() => {
    Handler.forwardTppTransfers = jest.fn().mockResolvedValue()
  })
  ...
  describe('POST', () => {
    ...
    it('handles when forwardTppTransfers throws error', async () => {
      // Generate request
      const request = await Mockgen.generateRequest(path, method, resource, Config.PROTOCOL_VERSIONS)
      ...
      Handler.forwardTppTransfers.mockImplementation(async () => { throw err })
      ...
      // Assert
      expect(response.statusCode).toBe(202)
      expect(Handler.forwardTppTransfers).toHaveBeenCalledTimes(1)
      expect(Handler.forwardTppTransfers.mock.results[0].value).rejects.toThrow(err)
    })
    ...
    it('returns an error response and logs when getSpanTags throws', async () => {
      ...
      // The handler re-formats and re-throws as an FSPIOP error; assert non-202 and that we logged the error
