...
const TppTransfers = require('../../../src/domain/tppTransfers')
...
describe('TppTransfers', () => {
  // URI
  const resource = 'TppTransfers'
  ...
  describe('forwardTppTransfers', () => {
    it('forwards a GET request when the payload is undefined', async () => {
      ...
      const options = [
        Enum.EndPoints.FspEndpointTemplates.TPP_TRANSFERS_GET,
      ...
      // Act
      const result = await TppTransfers.forwardTppTransfers(...options)

      // Assert
      expect(result).toBe(true)
    })

    it('forwards a POST request with a payload', async () => {
      // Arrange
      sandbox.stub(Endpoint, 'getEndpoint').resolves('http://localhost:3000')
      sandbox.stub(Request, 'sendRequest').resolves({
        ok: true,
      ...
      const options = [
        Enum.EndPoints.FspEndpointTemplates.TPP_TRANSFERS_POST,
        TestHelper.defaultHeaders(resource, Config.PROTOCOL_VERSIONS),
        'post',
        { },
        { executionRequestId: 'abcd', transactionRequestId: 'efgh' },
        SpanMock
      ]
    ...
    it('handles when the endpoint could not be found', async () => {
      ...
      sandbox.stub(TppTransfers, 'forwardTppTransfersError').resolves({})
      sandbox.stub(Request, 'sendRequest').resolves({
        ok: true,
        status: 202,
        statusText: 'Accepted'
      ...
      // Act
      const action = async () => TppTransfers.forwardTppTransfers(...options)

      // Assert
      await expect(action()).rejects.toThrow(/No FSPIOP_CALLBACK_URL_TPP_REQ_SERVICE endpoint found for tppTransfers abcd for dfsp1/)
    })
    ...
    it('handles when the the request fails and span is undefined', async () => {
      ...
      sandbox.stub(Endpoint, 'getEndpoint').resolves('http://localhost:3000')
      sandbox.stub(Request, 'sendRequest').throws(ErrorHandler.Factory.createFSPIOPError(ErrorHandler.Enums.FSPIOPErrorCodes.DESTINATION_COMMUNICATION_ERROR, 'Failed to send HTTP request to host', new Error(), '', [{ key: 'cause', value: {} }]))
      sandbox.stub(TppTransfers, 'forwardTppTransfersError').resolves(true)
      const options = [
        Enum.EndPoints.FspEndpointTemplates.TPP_TRANSFERS_GET,
        TestHelper.defaultHeaders(resource, Config.PROTOCOL_VERSIONS),
        'get',
  ...
  describe('forwardTppTransfersError', () => {
    it('sends the error request ', async () => {
      ...
      const options = [
        ...
        Enum.EndPoints.FspEndpointTemplates.TPP_TRANSFERS_PUT_ERROR,
      ...
      // Act
      const result = await TppTransfers.forwardTppTransfersError(...options)
    ...
    it('handles a missing executionRequestId', async () => {
      // Arrange
      sandbox.stub(Endpoint, 'getEndpoint').resolves('http://localhost:3000')
      sandbox.stub(Request, 'sendRequest').resolves({
        ok: true,
    ...
    it('handles when the request fails', async () => {
      ...
      // Act
      const action = async () => TppTransfers.forwardTppTransfersError(...options)
