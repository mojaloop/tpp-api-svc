...
const url = '/tppAccountsRequest/12345678-1234-4abc-9abc-123456789012'

describe('PUT /tppAccountsRequest/{ID}', () => {
  let server: Server
  ...
  it('returns 400 when authChannel is not in the allowed enum', async () => {
    const response = await server.inject({
      ...
      url,
      headers: validHeaders,
      payload: { ...validPayload, authChannel: 'BOGUS' }
    })
...
describe('PUT /tppAccountsRequest/{ID}/error', () => {
  let server: Server
