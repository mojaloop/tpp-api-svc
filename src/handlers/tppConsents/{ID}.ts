...
module.exports = {
  ...
  get: async (context: any, request: TraceableRequest, h: ResponseToolkit) => {
    ...
    try {
      ...
      tppConsents.forwardTppConsents(Enum.EndPoints.FspEndpointTypes.TPP_CB_URL_CONSENTS_GET, request.headers, Enum.Http.RestMethods.GET, request.params, request.payload, span).catch((err: Error) => {
   ...
   * responses: 202, 400, 401, 403, 404, 405, 406, 501, 503
   */
  delete: async (context: any, request: TraceableRequest, h: ResponseToolkit) => {
    const histTimerEnd = Metrics.getHistogram(
      'tpp_consents_delete',
    ...
    try {
      ...
      tppConsents.forwardTppConsents(Enum.EndPoints.FspEndpointTypes.TPP_CB_URL_CONSENTS_DELETE, request.headers, Enum.Http.RestMethods.DELETE, request.params, request.payload, span).catch((err: Error) => {
        // Do nothing with the error - forwardTppConsents takes care of async errors
        request.server.log(['error'], `ERROR - forwardTppConsents: ${LibUtil.getStackOrInspect(err)}`)
      })
      histTimerEnd({ success: true })
