...
const tppConsentRequest = require('../../domain/tppConsentRequest')
...
module.exports = {
   ...
   * description: The HTTP request `GET /tppConsentRequests/{ID}` is used to get information about a previously
  ...
  get: async (context, request, h) => {
    ...
    try {
      ...
      tppConsentRequest.forwardTppConsentRequest(Enum.EndPoints.FspEndpointTemplates.TPP_CONSENT_REQUESTS_GET, request.headers, Enum.Http.RestMethods.GET, request.params, request.payload, span).catch(err => {
   ...
   * description: A DFSP uses this callback to (1) inform the PISP that the consentRequest has been accepted, and (2) communicate to the PISP which `authChannel` it should use to authenticate their user with.
  ...
  put: async (context, request, h) => {
    ...
    try {
      ...
      tppConsentRequest.forwardTppConsentRequest(Enum.EndPoints.FspEndpointTemplates.TPP_CONSENT_REQUESTS_PUT, request.headers, Enum.Http.RestMethods.PUT, request.params, request.payload, span).catch(err => {
        // Do nothing with the error - forwardTppConsentRequest takes care of async errors
        request.server.log(['error'], `ERROR - forwardTppConsentRequest: ${LibUtil.getStackOrInspect(err)}`)
      })
      histTimerEnd({ success: true })
  ...
  patch: async (context, request, h) => {
    ...
    try {
      ...
      tppConsentRequest.forwardTppConsentRequest(Enum.EndPoints.FspEndpointTemplates.TPP_CONSENT_REQUESTS_PATCH, request.headers, Enum.Http.RestMethods.PATCH, request.params, request.payload, span).catch(err => {
        // Do nothing with the error - forwardTppConsentRequest takes care of async errors
        request.server.log(['error'], `ERROR - forwardTppConsentRequest: ${LibUtil.getStackOrInspect(err)}`)
      })
      histTimerEnd({ success: true })
