...
module.exports = {
  ...
  put: async (context, request, h) => {
    ...
    try {
      ...
      tppConsentRequest.forwardTppConsentRequestError(request.headers, request.headers['fspiop-destination'], Enum.EndPoints.FspEndpointTemplates.TPP_CONSENT_REQUESTS_PUT_ERROR, Enum.Http.RestMethods.PUT, request.params.ID, request.payload, span).catch(err => {
        // Do nothing with the error - forwardTppConsentRequestError takes care of async errors
        request.server.log(['error'], `ERROR - forwardTppConsentRequestError: ${LibUtil.getStackOrInspect(err)}`)
      })
