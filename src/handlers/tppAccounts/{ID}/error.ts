...
module.exports = {
  ...
  put: async (context: any, request: TraceableRequest, h: ResponseToolkit) => {
    ...
    try {
      ...
      tppAccounts.forwardTppAccountsError(request.headers, request.headers['fspiop-destination'], Enum.EndPoints.FspEndpointTemplates.TPP_ACCOUNTS_PUT_ERROR, Enum.Http.RestMethods.PUT, request.params.ID, request.payload, span).catch((err: Error) => {
